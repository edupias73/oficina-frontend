...
export class ClientesComponent implements OnInit {
  ...
  // --- LISTAGEM ---
  carregarClientes() {
    this.http.get<any[]>('[https://oficina-backend-production-1f8e.up.railway.app](https://oficina-backend-production-1f8e.up.railway.app)/clientes').subscribe({
      next: (dados) => {
  ...
  salvarNovoCliente() {
    ...
    this.http.post('[https://oficina-backend-production-1f8e.up.railway.app](https://oficina-backend-production-1f8e.up.railway.app)/clientes', dadosParaEnviar).subscribe({
      next: () => {
  ...
  salvarEdicao() {
    this.http.put('[https://oficina-backend-production-1f8e.up.railway.app](https://oficina-backend-production-1f8e.up.railway.app)/clientes', this.clienteEmEdicao).subscribe({
      next: () => {
  ...
  excluirCliente(id: number) {
    if (confirm('Tem certeza? Isso pode apagar histórico de O.S.!')) {
      this.http.delete(`[https://oficina-backend-production-1f8e.up.railway.app](https://oficina-backend-production-1f8e.up.railway.app)/clientes/${id}`).subscribe({
