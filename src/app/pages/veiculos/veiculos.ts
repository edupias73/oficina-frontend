...
export class VeiculosComponent implements OnInit {
  ...
  carregarFrota() {
    this.http.get<any[]>('https://oficina-backend-production-1f8e.up.railway.app/veiculos').subscribe({
      next: (dados) => {
  ...
  carregarClientes() {
    this.http.get<any[]>('https://oficina-backend-production-1f8e.up.railway.app/clientes').subscribe(d => this.listaClientes = d);
  }
  ...
  salvarNovoVeiculo() {
    ...
    this.http.post('https://oficina-backend-production-1f8e.up.railway.app/veiculos', this.dadosNovoVeiculo).subscribe({
      next: () => {
  ...
  salvarEdicao() {
    this.http.put('https://oficina-backend-production-1f8e.up.railway.app/veiculos', this.veiculoEmEdicao).subscribe({
      next: () => {
  ...
  excluirVeiculo(id: number) {
    if(confirm('Excluir este veículo?')) {
      this.http.delete(`https://oficina-backend-production-1f8e.up.railway.app/veiculos/${id}`).subscribe(() => this.carregarFrota());
    }
  }
}