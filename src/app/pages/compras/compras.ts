...
export class ComprasComponent implements OnInit {
  ...
  carregarCompras() {
    this.http.get<any[]>('https://oficina-backend-production-1f8e.up.railway.app/compras').subscribe(dados => { this.listaCompras = dados; this.cd.detectChanges(); });
  }

  carregarFornecedores() {
    this.http.get<any[]>('https://oficina-backend-production-1f8e.up.railway.app/fornecedores').subscribe(dados => this.listaFornecedores = dados);
  }

  carregarProdutos() {
    this.http.get<any[]>('https://oficina-backend-production-1f8e.up.railway.app/produtos').subscribe(dados => { this.listaProdutos = dados; this.produtosFiltrados = dados; });
  }
...
abrirModalEdicao(resumo: any, event?: Event) {
    ...
    this.http.get<any>(`https://oficina-backend-production-1f8e.up.railway.app/compras/${resumo.id}`).subscribe({
      next: (compra) => {
        this.novaCompra = {
          id: compra.id,
...
fecharModal() {
 ...
 salvarRascunho(silencioso: boolean = false) {
    ...
    if (this.novaCompra.id) {
      this.http.put(`https://oficina-backend-production-1f8e.up.railway.app/compras/${this.novaCompra.id}`, dadosParaOJava).subscribe({
      ...
      this.http.post('https://oficina-backend-production-1f8e.up.railway.app/compras', dadosParaOJava).subscribe({
  ...
  efetivarCompra(id: number, event: Event) {
    ...
    if (confirm('Lançar as peças no estoque e alterar preços de custo?')) {
      this.http.put(`https://oficina-backend-production-1f8e.up.railway.app/compras/${id}/efetivar`, {}).subscribe({
  ...
  excluirCompra(id: number, event: Event) {
    ...
    if (confirm('Excluir rascunho?')) {
      this.http.delete(`https://oficina-backend-production-1f8e.up.railway.app/compras/${id}`).subscribe({ next: () => this.carregarCompras() });
    }
  }
  ...
  salvarNovoProduto() {
    ...
    this.http.post<any>('https://oficina-backend-production-1f8e.up.railway.app/produtos', this.novoProduto).subscribe({
      next: (produtoCriado) => {
        this.carregarProdutos();
